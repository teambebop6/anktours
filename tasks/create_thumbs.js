// Create thumbnails
// example: node createThumbs from to 100
...
utils.ensureDirExists(destination_path, function(err){
	...
	image_files.forEach(function(image){
		...
		.write(path.join(destination_path, "img" + path.basename(image)), function(err){
	...
	easyimg.resize({
		src: image,
		dst: path.join(destination_path, path.basename(image)),
		width: process.argv[2], height: process.argv[2],
		quality: 100
	}).then(
		function(image){
